...
const hadleUpdateContainerStats = async (mongoInstance, host_id, container_name, stats) => {
    try {
        ...
        await containers.updateOne({host_id: host_id, name: container_name}, {$set: stats}, {upsert: true})
        return true
    } catch (error) {
        console.log(error);
...
const handleUpdateContainerLogs = async (mongoInstance, conf_id, container_name, logs) => {
    try {
        ...
        await containers.updateOne({container_id: conf_id}, {$set: {conf_id: conf_id, log: logs}}, {upsert: true})
        return true
    } catch (error) {
        console.log(error);
        await Promise.reject(error)
